...
const SmartAoE = (() => {
 ...
 //---------------------------------------------------------------------------------------
 // TokenMod helpers â€” bar changes via TokenMod so ScriptCards Concentration trigger fires
 //---------------------------------------------------------------------------------------
 const getGMPlayerId = function() {
 const gmPlayer = findObjs({ _type: 'player' }).find(p => playerIsGM(p.get('_id')));
 return gmPlayer ? gmPlayer.get('_id') : null;
 };

 const sendTokenModSet = function(tokID, setArgs) {
 const gmId = getGMPlayerId();
 if (!gmId || setArgs.length === 0) {
 if (!gmId) {
 log(`SmartAoE: Could not find a GM player to execute TokenMod.`);
 }
 return false;
 }
 sendChat(scriptName, `!token-mod --api-as ${gmId} --ids ${tokID} --set ${setArgs.join(' ')}`);
 return true;
 };
 ...
 const bringControlTokensForward = function(link) {
 if (!link) { return; }
 const ids = [link.controlTokID];
 if (link.originTokID && link.originTokID !== link.controlTokID) {
 const oTok = getObj('graphic', link.originTokID);
 if (oTok && oTok.get('name') === 'AoEControlToken') {
 ids.push(link.originTokID);
 }
 }
 const gmId = getGMPlayerId();
 if (gmId) {
 sendChat(scriptName, `!token-mod --ignore-selected --api-as ${gmId} --ids ${ids.join(',')} --order|tofront`);
 return;
 }
 ids.forEach(id => {
 ...
 const applyAoELayering = function(link) {
 if (!link) { return; }
 sendPathsToBackSerial(link.pathIDs, 50, () => bringControlTokensForward(link));
 };

 const layerAoEControlTokens = function(link) {
 applyAoELayering(link);
 ...
 */
 ...
 let actionText = `/w gm &{template:default}{{name=AoE Generator}} {{${arrow}${arrow}&nbsp;***CAST AT RANGE*** ${arrow}${arrow}Â \n` +
 `**_____Square / Cube______**Â \n` +
 ...
 `------------------------------------------\n` +
 `((${explosion})) ***FROM CASTER*** ((${explosion}))Â Â \n` +
 `**_____Square / Cube______**Â \n` +
 ...
 `[40ft](~AoEControlToken|40ft-Square-Caster) [Variable](~AoEControlToken|Variable-Square)Â Â Â Â Â Â Â Â Â Â \n` +
 `**_____Circle / Sphere______**Â \n` +
 ...
 `[40ft](~AoEControlToken|40ft-Circle-Caster) [Variable](~AoEControlToken|Variable-Circle)Â Â Â Â Â Â Â Â Â Â \n` +
 `**____PF Circle / Sphere____**Â \n` +
 ...
 `[40ft](~AoEControlToken|40ft-PF-Circle-Caster) [Variable](~AoEControlToken|Variable-PF-Circle)Â Â Â Â Â Â Â Â Â Â \n` +
 `**__________Line__________**Â \n` +
 ...
 sendChat(scriptName,`${whisperString} Unable to find imgsrc for default token of \(${baseObj.name}\) You must use an image file that has been uploaded to your Roll20 Library.`)
 return;
 }
 ...
 */
 ...
 const build5eCone = function(rad, z, coneWidth, coneDirection, triangleOnly=false) {
 let pointsJSON = '';
 ...
 */
 ...
 // Triangle-only 5e cone outline positioned in world space (no phantom bbox points).
 const build5eConeOutlineWorld = function(originPt, rad, z, coneWidth, coneDirection) {
 let deg2rad = Math.PI/180;
 coneDirection = normalizeTo360deg(coneDirection);
 let th1 = deg2rad * (coneDirection - coneWidth / 2);
 let th2 = deg2rad * (coneDirection + coneWidth / 2);
 let apex = originPt;
 let pt1 = get5eConePt(originPt, rad + z, th1);
 let pt2 = get5eConePt(originPt, rad + z, th2);

 let minX = Math.min(apex.x, pt1.x, pt2.x);
 let maxX = Math.max(apex.x, pt1.x, pt2.x);
 let minY = Math.min(apex.y, pt1.y, pt2.y);
 let maxY = Math.max(apex.y, pt1.y, pt2.y);
 let width = Math.max(maxX - minX, 1);
 let height = Math.max(maxY - minY, 1);
 let centerX = (minX + maxX) / 2;
 let centerY = (minY + maxY) / 2;

 let lApex = new pt(apex.x - minX, apex.y - minY);
 let l1 = new pt(pt1.x - minX, pt1.y - minY);
 let l2 = new pt(pt2.x - minX, pt2.y - minY);

 let pathstring = `[[\"M\",${lApex.x},${lApex.y}],[\"L\",${l1.x},${l1.y}],[\"L\",${l2.x},${l2.y}],[\"L\",${lApex.x},${lApex.y}]]`;

 return { pathstring, left: centerX, top: centerY, width, height };
 };
 ...
 */
 ...
 //Face-center origin for cones: midpoint of the token edge facing the control point.
 const getFaceCenterOriginPt = function(oPt, oWidth, oHeight, cPt) {
 const dx = cPt.x - oPt.x;
 const dy = cPt.y - oPt.y;
 if (Math.abs(dx) >= Math.abs(dy)) {
 return new pt(oPt.x + (dx >= 0 ? oWidth / 2 : -oWidth / 2), oPt.y);
 }
 ...
 aoeLinks.links[a].pathIDs = newPaths;
 layerAoEControlTokens(aoeLinks.links[a]);
 }
 }
 ...
 msgList[0].content.split(" ").forEach((str, n) => {
 ...
 if (!tok) {
 return `Unable to find token ${name}`;
 }

 // bar is an array of bar numbers. Damage cascades from the first bar into subsequent bars.
 if (bar.length > 0) {
 bar.forEach(b => {
 ...
 let appliedDamage = Math.min(currentHP[i], remainingDamage);
 newHP[i] = Math.max(currentHP[i] - appliedDamage, 0);
 ...
 let remainingHealing = Math.abs(damage);

 for (let i = 0; i < bar.length; i++) {
 let availableHealing = Math.max(maxHP[i] - currentHP[i], 0);
 let appliedHealing = Math.min(availableHealing, remainingHealing);
 newHP[i] = currentHP[i] + appliedHealing;
 amountHealed += appliedHealing;
 ...
 let atMax = amountHealed > 0 && bar.every((b, i) => newHP[i] >= maxHP[i]);
 returnString = atMax
 ? `Healed ${name} by ${amountHealed} (@Max)`
 : `Healed ${name} by ${amountHealed}`;
 }

 // Remove token if all configured bars have reached zero.
 let atZero = bar.length > 0 && bar.every((b, i) => newHP[i] === 0);
 if (removeAtZero && atZero) {
 removeToken(tokID);
 return returnString;
 }

 // Apply bar changes through TokenMod so ScriptCards Concentration trigger fires.
 let tokenModChanges = [];
 bar.forEach((b, i) => {
 let difference = newHP[i] - currentHP[i];
 if (difference !== 0) {
 let sign = difference > 0 ? '+' : '';
 tokenModChanges.push(`bar${b}_value|${sign}${difference}`);
 }
 });

 if (tokenModChanges.length > 0) {
 sendTokenModSet(tokID, tokenModChanges);
 }

 // Zero HP marker after bar update so Concentration sees damage before dead marker.
 let finalBarIndex = bar.length - 1;
 if (zeroHPmarker && bar.length > 0 && newHP[finalBarIndex] === 0) {
 addStatusMarkers(tokID, zeroHPmarker);
 } else if (zeroHPmarker && bar.length > 0 && newHP[finalBarIndex] > 0) {
 removeStatusMarkers(tokID, zeroHPmarker);
 }
 ...
 */
 ...
 autoApplyMsg = `${whisperString} ${publicDamageMessages.join(' ')}`;
 ...
 autoApplyMsg = '/w gm ' + gmDamageMessages.join(' ');
 sendChat(scriptName, autoApplyMsg);
 }
 }

 const sendErrorMessage = function (err) {
 const errorMessage = "Something went wrong when processing rolls. " +
 `The error message generated by Roll20 is: ${err}`;
 ...
 clearCache('temp');
 sendChat(scriptName, `${whisperString} The SmartAoE cache has been cleared and all AoEs have been unlinked! Any active AoEs in the campaign will have to be deleted manually.`, null, {
 noarchive: true
 });
 }
 ...
 */
 ...
 //--------------------------------------------------------------------
 // Trigger AoE Effect !smarttrigger 
 //--------------------------------------------------------------------
 ...
 if (ig.length < 2) {
 throw 'Invalid argument syntax. Structure is --ignore|attrName,value';
 //sendChat(scriptName,`/w "${who}" `+ 'Invalid argument syntax. Structure is --ignore|attrName,value');
 } else {
 ignoreAttr = ig[0];
 ignoreVal = ig[1];
 }
 ...
 if (oTok===undefined) {
 sendChat(scriptName, 'Unable to find the origin token from the selectedID If a characterID was passed, check the player ribbon page.');
 return;
 }
 }
 }
 ...
 sendChat(scriptName, `${whisperString} Not enough ${resourceAlias}! Current ${resourceAlias} = ${resourceCurrent}, Cost = ${resourceCost}`, null, {noarchive:true});
 return;
 } else {
