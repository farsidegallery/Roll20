...
------------------------*/
    ...
    //**CHECK BARS------------//
        ...
        //**CHECK MONSTER OR PLAYER------------//
                ...
                var colortype = (state.HealthColors.auraTint) ? 'tint' : 'aura2';
            //IF PLAYER------------
                if(type == 'Player') {
...
------------------------*/
    //SET TOKEN COLORS------------
        TokenSet = function (obj, sizeSet, markerColor, pColor, update) {
            ...
            if(state.HealthColors.auraTint === true) {
                if(obj.get('aura2_color') == markerColor && update === "YES") {
                    obj.set({'aura2_color': "transparent",});
                }
    ...
    //REMOVE ALL------------
        SetAuraNone = function (obj) {
            ...
            else obj.set({'aura2_color': "transparent",});
        },
    //FORCE ALL TOKEN UPDATE------------
        MenuForceUpdate = function(){
            ...
            const drainQueue = ()=>{
                ...
                if(t){
                    if (state.HealthColors.auraColorOn === true) {
    ...
    //REGISTER TRIGGERS------------
        registerEventHandlers = function () {
            ...
            if('undefined' !== typeof SmartAoE && SmartAoE.ObserveTokenChange) {
                log('ATHC registering SmartAoE.ObserveTokenChange')
            ...
            if('undefined' !== typeof ScriptCards && ScriptCards.ObserveTokenChange) {
                log('ATHC registering ScriptCards.ObserveTokenChange')
