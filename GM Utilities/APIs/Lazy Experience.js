...
var LazyExperience = LazyExperience || (function() {
    ...
    handleInput = (msg) => {
        ...
        // ScriptCards --@ calls have no player id; allow death hook without GM check.
        const scriptCallable = (extracommand === 'npcdeath' || extracommand === 'death');
        if (!scriptCallable && !playerIsGM(msg.playerid)) return;
        ...
        switch(extracommand){
                ...
                case 'npcdeath':
                case 'death':
                    handleNpcDeathByTokenId(args.shift());
                break;
    ...
    shouldProcessDeath = (tokenId) => {
        let now = Date.now();
        if(recentDeaths[tokenId] && (now - recentDeaths[tokenId]) < DEATH_DEDUP_MS) return false;
        recentDeaths[tokenId] = now;
        return true;
    },

    promptNpcDeathXP = (tokenObj) => {
        if(!tokenObj) return;

        let tokenId = tokenObj.get('_id');
        if(!shouldProcessDeath(tokenId)) return;

        let characterId = tokenObj.get('represents');
        if(!characterId) return;

        let experience = getAttrByName(characterId, state[state_name].config.npc_experience_attribute_name, 'current') * 1;
        if(isNaN(experience)) experience = 0;

        if(experience > 0){
            let yesButton = makeButton('Yes', '!' + state[state_name].config.command + ' add session '+experience, buttonStyle2);
            makeAndSendMenu('<b>' + tokenObj.get('name') + '</b> just died, do you want to add <b>'+experience+'</b> xp to the threshold?<br>'+yesButton, '', 'gm');
        }
    },

    handleNpcDeathByTokenId = (tokenId) => {
        if(!tokenId) return;
        promptNpcDeathXP(getObj('graphic', tokenId));
    },

    handleStatusmarkerChange = (obj, prev) => {
        if(!obj || obj.get('_subtype') !== 'token') return;
        let marker = state[state_name].config.marker;
        if(!markerWasJustApplied(obj, prev, marker)) return;
        promptNpcDeathXP(obj);
    },
    ...
    sendHelpMenu = (first) => {
        ...
        let listItems = [
            ...
            '<span style="text-decoration: underline">!'+state[state_name].config.command+' npcdeath TOKEN_ID</span> - Prompt to add an NPC\'s xp value to session xp (also used by Concentration trigger).',
        ]
    ...
    registerEventHandlers = () => {
		...
		if('undefined' !== typeof Corpsecart && Corpsecart.ObserveTokenChange){
            Corpsecart.ObserveTokenChange(function(obj,prev){
                handleStatusmarkerChange(obj,prev);
            });
        }
    };
    ...
    return {
        ...
        HandleNpcDeath: handleNpcDeathByTokenId
    };
})();
