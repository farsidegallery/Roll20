 ...
 * Changes in this version 2.50 by FarsideGallery using Cursor AI
 * Changes in this version 2.51 by FarsideGallery using Cursor AI (AuraTriggers turn sync)
 * Changes in this version 2.52 by FarsideGallery using Cursor AI (stock AuraTriggers compatibility; ScriptCards-style turnorder set)
...
//*************************************************************************************************************
    ...
    registerTrackedCondition = function (tokenObj, configCondition, duration, message) {
        if (!tokenObj || !configCondition) {
            return false;
        }

        let tokenId = tokenObj.get('_id');

        if (findAssignedCondition(tokenId, configCondition.key).length > 0) {
            return false;
        }

        if (duration === null || typeof duration === 'undefined' || duration === '') {
            duration = parseInt(configCondition.duration, 10);
        } else {
        ...
        let direction = parseInt(configCondition.direction, 10);
        if (isNaN(direction)) {
            direction = 0;
        }

        state[combatState].conditions.push({
            id: tokenId,
            key: configCondition.key,
            name: configCondition.name,
            icon: configCondition.icon,
            iconType: configCondition.iconType,
            addMacro: configCondition.addMacro,
            addPersistentMacro: configCondition.addPersistentMacro,
            concentration: configCondition.concentration,
            override: configCondition.override,
            message: (message && message !== 'None') ? message : configCondition.message,
            type: configCondition.type,
            targeted: configCondition.targeted,
            targetedAPI: configCondition.targetedAPI,
            target: [],
            tokenConditionID: null,
            duration: duration,
            direction: direction
        });

        if (debug) {
            log('Register Tracked Condition: ' + configCondition.key + ' duration ' + duration + ' from token marker');
        }
    ...
    syncExternalMarkersToState = function (tokenObj) {
        ...
        returnMarkers(tokenObj).forEach(function (marker) {
            ...
            let configCondition = getConditionByMarker(marker);
            if (!configCondition) {
                return;
            }
            ...
            registerTrackedCondition(tokenObj, configCondition, getMarkerBadge(marker));
        });
    },
...
//*************************************************************************************************************
    ...
    getMarkerBadge = function (marker) {
        let match = (marker || '').match(/@(\d+)$/);
        return match ? parseInt(match[1], 10) : null;
    },

    markerBasePresent = function (markers, base) {
        if (!base) {
            return false;
        }
        return markers.some(function (marker) {
            return getMarkerBase(marker) === base;
        });
    },
    ...
    updateConditionMarker = function (tokenObj, condition) {
        ...
        if (debug) {
            log('Update Condition Marker: ' + condition.key + ' duration ' + condition.duration + ' direction ' + condition.direction)
        }
            ...
            if(obj.get('statusmarkers') !== prev.statusmarkers){
                ...
                if (newstatusmarkers.length > 0 ) {
                    newstatusmarkers.forEach(function(marker){
                        ...
                        let condition = getConditionByMarker(marker)
                        if(!condition || !base) return;
                        if(!markerBasePresent(prevstatusmarkers, base)){
                            registerTrackedCondition(obj, condition, getMarkerBadge(marker), condition.message);
                        }
                    });
                }
            }
...
//TURNORDER
//*************************************************************************************************************
    removeTokenFromCombat = function (tokenId) {
        if (debug) {
            log('Remove Token From Combat')
        }

        let markerId = getOrCreateMarker().get('id');
        let nextMarkerId = getOrCreateMarker(true).get('id');

        if (!tokenId || tokenId === '-1' || tokenId === markerId || tokenId === nextMarkerId) {
            return;
        }

        let tokenObj = getObj('graphic', tokenId);
        let keys = [...new Set(state[combatState].conditions.filter(c => c.id === tokenId).map(c => c.key))];

        if (tokenObj) {
            keys.forEach(key => removeConditionFromToken(tokenObj, key, true));
        } else {
            state[combatState].conditions = state[combatState].conditions.filter(c => c.id !== tokenId);
        }
    ...
    syncTurnorderRemovals = function (prevTurnorder, turnorder) {
        if (debug) {
            log('Sync Turnorder Removals')
        }

        let markerId = getOrCreateMarker().get('id');
        let nextMarkerId = getOrCreateMarker(true).get('id');
        let currIds = turnorder.map(t => t.id);
        let removedIds = [];

        prevTurnorder.forEach(turn => {
            let id = turn.id;
            if (!id || id === '-1' || id === markerId || id === nextMarkerId) {
                return;
            }
            if (!currIds.includes(id) && !removedIds.includes(id)) {
                removedIds.push(id);
                removeTokenFromCombat(id);
            }
        });
    ...
    handleTurnorderChange = function (obj, prev) {
        ...
        let removedIds = [];
        if (prevTurnorder.length && turnorder.length) {
            removedIds = syncTurnorderRemovals(prevTurnorder, turnorder);
        }

        if (state[combatState].cmDrivingTurnorder) {
            return;
        }
    ...
    decodeAuraGmnotes = function (input) {
        let decoded, auraInfo;
        ...
        decoded = decoded.replace(/&(#\d+|#x[0-9a-fA-F]+|[a-zA-Z]+);/g, function (match, entity) {
    ...
    getTokenPixelCoords = function (token) {
        return {
            x: token.get('left'),
            y: token.get('top'),
    ...
    getPageAuraScale = function (pageId) {
        let pageGraphics = findObjs({ _type: 'graphic', _pageid: pageId }),
            mapIncrement = 5;

        if (pageGraphics.length) {
    ...
    isWithinRoundAura = function (t1, t2, auraRadius) {
        let dx = t1.x - t2.x,
            dy = t1.y - t2.y,
            distanceSquared = dx * dx + dy * dy,
            r1 = t1.width / 2,
            r2 = t2.width / 2,
            maxDistance = Math.floor(r2 + auraRadius + r1);

        return distanceSquared < (maxDistance * maxDistance);
    },

    isWithinSquareAura = function (t1, t2, auraRadius) {
        let r1 = t1.width / 2,
            r2 = t2.width / 2,
            auraLeft = t2.x - r2 - auraRadius,
            auraRight = t2.x + r2 + auraRadius,
            auraTop = t2.y - r2 - auraRadius,
            auraBottom = t2.y + r2 + auraRadius,
            closestX = Math.max(auraLeft, Math.min(t1.x, auraRight)),
            closestY = Math.max(auraTop, Math.min(t1.y, auraBottom)),
            dx = t1.x - closestX,
            dy = t1.y - closestY;
    ...
    getAurasByColor = function (auras, color) {
        let matches = [];
        ...
        auras.forEach(function (aura, index) {
            if (aura && aura.color === color) {
                matches.push(_.extend({}, aura, { _jsonIndex: index }));
            }
        });
    ...
    tokenMatchesAuraTarget = function (token, auraSourceId, auraAction) {
        let isCharacter, theCharacter, isPC, layer, toLayers;

        if (!token || !auraAction) {
            return false;
        }

        if (token.get('_id') === auraSourceId) {
            return auraAction.applySelf === true;
        }

        isCharacter = token.get('represents') ? true : false;
        theCharacter = isCharacter ? getObj('character', token.get('represents')) : null;
        isPC = theCharacter ? (theCharacter.get('controlledby') !== '') : false;

        if (isCharacter && !isPC && auraAction.toNPCs === false) {
            return false;
        }

        if (isPC && auraAction.toPCs === false) {
            return false;
        }

        if (!isCharacter && auraAction.toGraphics !== true) {
            return false;
        }

        layer = token.get('layer');
        toLayers = auraAction.toLayers || 'objects,token';

        if (layer === 'objects' && toLayers.indexOf('objects') === -1 && toLayers.indexOf('token') === -1) {
            return false;
        }

        if (layer === 'gmlayer' && toLayers.indexOf('gmlayer') === -1) {
            return false;
        }

        if (layer === 'map' && toLayers.indexOf('map') === -1) {
            return false;
        }

        if (layer === 'walls' && toLayers.indexOf('walls') === -1) {
            return false;
        }

        if (layer === 'foreground' && toLayers.indexOf('foreground') === -1) {
            return false;
        }
    ...
    replaceAuraChatVars = function (text, auraSource, auraName, targetToken) {
        if (typeof text !== 'string') {
            return '';
        }

        return text.replace(/\[([A-Z0-9_]+)\]/gi, function (match, key) {
            let normalizedKey = key.toUpperCase(),
                values = {
                    TNAME: targetToken.get('name'),
                    ANAME: auraName,
                    ATNAME: auraSource.get('name'),
                    TID: targetToken.get('_id'),
                    ATID: auraSource.get('_id')
                };

            if (values[normalizedKey] !== undefined && values[normalizedKey] !== null) {
    ...
    fireAuraTurnChatActionsForToken = function (token, chatActionKey) {
        let pageId, targetCoords, mapIncrement, auraSources;
        ...
        targetCoords = getTokenPixelCoords(token);
        mapIncrement = getPageAuraScale(pageId);
        auraSources = findObjs({ _type: 'graphic', _pageid: pageId });

        auraSources.forEach(function (source) {
            let aura1Radius, aura2Radius, gmnotes, auraParsed, sourceCoords, auraActions, square, radiusPixels, inAura, chatAction;

            aura1Radius = source.get('aura1_radius');
            aura2Radius = source.get('aura2_radius');

            if (Math.abs(aura1Radius) <= 0 && Math.abs(aura2Radius) <= 0) {
                return;
            }

            gmnotes = source.get('gmnotes');
            auraParsed = decodeAuraGmnotes(gmnotes);

            if (!auraParsed) {
                return;
            }

            sourceCoords = getTokenPixelCoords(source);

            if (Math.abs(aura1Radius) > 0) {
                auraActions = getAurasByColor(auraParsed, source.get('aura1_color'));
                square = source.get('aura1_square');
                radiusPixels = (mapIncrement !== 0) ? (aura1Radius / mapIncrement * 70) : 0;

                auraActions.forEach(function (auraAction) {
                    chatAction = auraAction[chatActionKey];

                    if (!chatAction || !tokenMatchesAuraTarget(token, source.get('_id'), auraAction)) {
                        return;
                    }

                    inAura = square
                        ? isWithinSquareAura(targetCoords, sourceCoords, radiusPixels)
                        : isWithinRoundAura(targetCoords, sourceCoords, radiusPixels);

                    if (!inAura) {
                        return;
                    }

                    sendChat('AuraTriggers', replaceAuraChatVars(chatAction, source, auraAction.name, token));
                });
            }

            if (Math.abs(aura2Radius) > 0) {
                auraActions = getAurasByColor(auraParsed, source.get('aura2_color'));
                square = source.get('aura2_square');
                radiusPixels = (mapIncrement !== 0) ? (aura2Radius / mapIncrement * 70) : 0;
    ...
    syncAuraTurnEvents = function (prevTurnorder, newTurnorder) {
        let prevToken, newToken;

        if (!Array.isArray(prevTurnorder) || !Array.isArray(newTurnorder)) {
            return;
        }

        if (!prevTurnorder.length || !newTurnorder.length) {
            return;
        }

        if (prevTurnorder[0].id === newTurnorder[0].id) {
            return;
        }

        prevToken = getObj('graphic', prevTurnorder[0].id);
        newToken = getObj('graphic', newTurnorder[0].id);

        if (prevToken) {
            fireAuraTurnChatActionsForToken(prevToken, 'chatActionOnEndTurn');
        }

        if (newToken) {
            fireAuraTurnChatActionsForToken(newToken, 'chatActionOnStartTurn');
        }
    },
    ...
    withCmTurnorderDrive = function (fn) {
        if (!state[combatState].cmTurnDriveDepth) {
            state[combatState].cmTurnDriveDepth = 0;
        }

        if (state[combatState].cmTurnDriveDepth === 0 && !state[combatState].auraSyncPrev) {
            state[combatState].auraSyncPrev = JSON.parse(JSON.stringify(getTurnorder()));
        }

        state[combatState].cmTurnDriveDepth++;
        state[combatState].cmDrivingTurnorder = true;

        try {
            fn();
        } finally {
            state[combatState].cmDrivingTurnorder = false;
            state[combatState].cmTurnDriveDepth--;

            if (state[combatState].cmTurnDriveDepth === 0) {
                scheduleAuraTurnSync();
            }
        }
    },

    deferTurnorderChange = function (prev, delay) {
        setTimeout(function () {
            doTurnorderChange(prev, delay);
        }, 0);
    },
...
//*************************************************************************************************************
    delayTurn = function () {
        withCmTurnorderDrive(function () {
            ...
            setTurnorder(turnorder);
            deferTurnorderChange(false, true);
        });
    },
...
//*************************************************************************************************************
    ...
    getAnnounceConditions = function (tokenObj, prev, delay, show) {
        syncExternalMarkersToState(tokenObj);

        if (debug) {
            log('Announce Condition')
            log('Tracked conditions on token: ' + state[combatState].conditions.filter(function (c) {
                return c.id == tokenObj.get('_id');
            }).length)
        }
        ...
        if (state[combatState].conditions) {
            state[combatState].conditions.forEach(condition => {
                let isTarget = Array.isArray(condition.target) && condition.target.includes(tokenId);
                let isOwner = condition.id == tokenId;

                if (!isOwner && !isTarget) {
                    return;
                }

                let duration = parseInt(condition.duration, 10)
                let direction = parseInt(condition.direction, 10)
                ...
                if (!isTarget && !delay && !show && direction !== 0) {
                    if (!prev) {
                ...
                if (duration <= 0 && direction !== 0) {
                ...
                } else if (duration > 0 && direction !== 0) {
                    output += '<div style="display:inline-block;"><strong>'+descriptionButton+'</strong> '+duration+' Rounds Left</div>';

                    if (!delay && !show && !isTarget) {
                        updateConditionMarker(tokenObj, condition)
                    }
                    ...
                    output += '<div style="display:inline-block;"><strong>'+descriptionButton+'</strong> '+duration+' Permanent</div>';
                    if (condition.hasOwnProperty('message')) {
                        if (condition.message != 'None' && condition.message.length > 0) {
                            output += '<div style="display:inline-block;"<strong>Message: </strong> '+condition.message+ '</div>';
                        }
                ...
                if (!removed) {
                    removeButton  = makeImageButton('!cmaster --remove,condition='+condition.key+',id='+tokenId,deleteImage,'Remove Condition','transparent',18)
